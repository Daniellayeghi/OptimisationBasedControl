...
int main(int argc, const char** argv)
{
    ...
    Eigen::Matrix<double, n_jpos + n_jvel, 1> k; k << 0, 0;
    Eigen::Matrix<float, n_jpos + n_jvel, 1> k_f = k.cast<float>();
    float input_data[] = {k_f(0,0), k_f(1, 0)};

    inputs.emplace_back(torch::from_blob(k_f.data(),{1, 2}));
    at::Tensor result = module.forward(inputs).toTensor();
//    torch::Tensor t = torch::zeros({1, 8}, torch::dtype(torch::kFloat32));
//    auto l = t.dim();
...
//    std::cout << t_a[0][0] << '\n';
    ...
    std::vector<at::Tensor> output_var;

    std::vector<at::Tensor> input_var(1);
    input_var.front() = torch::from_blob(input_data,{1, 2});
    input_var.front().set_requires_grad(true);

    auto module_wrapper  = [&](std::vector<at::Tensor>& inputs_var){
        auto i_a = inputs_var.front().accessor<float,2>();
        float input_data[] = {i_a[0][0], i_a[0][1]};
        inputs.front() = torch::from_blob(input_data, {1, 2});
    ...
    m = mj_loadXML("../../../models/pendulum.xml", 0, error, 1000);
    ...
    // setup cost params
    Eigen::Matrix<double, n_jpos + n_jvel, 1> x_desired; x_desired << M_PI, 0;
    ...
    for(auto element = 0; element < n_jpos; ++element)
    {
        x_gain(element + n_jpos,element + n_jpos) = 0 * 0.01;
    }
    ...
    // initial position
    d->qpos[0] =M_PI+1; d->qvel[0] = 0;

    Eigen::Matrix<double, n_ctrl, n_ctrl> R;
    Eigen::Matrix<double, n_jpos + n_jvel, n_jpos + n_jvel> Q;

    FiniteDifference<n_jpos + n_jvel, n_ctrl> fd(m);
    CostFunction<n_jpos + n_jvel, n_ctrl> cost_func(x_desired, u_desired, x_gain, u_gain, x_terminal_gain, m, &module);
