...
{
    using StateVec = Eigen::Matrix<double, 2, 1>;
    using StateMat = Eigen::Matrix<double, 2, 2>;
    using CtrlVec  = Eigen::Matrix<double, 1, 1>;
    using CtrlMat  = Eigen::Matrix<double, 1, 1>;
    auto state_gain = StateMat::Identity();
    auto ctrl_gain  = CtrlMat::Identity();

    struct Data{StateVec x = StateVec::Zero(); CtrlVec u = CtrlVec::Zero(); const double t = 0.01;};

    auto step = [](Data &d)
    {
        constexpr const double m = 1; double acc = 0;
        acc = d.u(0, 0) / m;
        d.x(0, 0) += acc* d.t;
        d.x(1, 0) += d.x(0, 0) * d.t;
    };
    ...
    constexpr const int time = 75; int samples = 300;
    std::vector<std::vector<double>> cst(samples, {0, 0, 0, 0, 0, 0, 0, 0});
    std::vector<Data> data_vec(samples, {StateVec::Zero(), CtrlVec::Zero()});
    Eigen::EigenMultivariateNormal<double> normX_cholesk (CtrlVec::Zero(),CtrlMat::Identity(),time,true);
    Eigen::Matrix<double, -1, -1> ctrl_samples; ctrl_samples.resize(10000, 75);
    GenericUtils::TimeBench timer("Basic_Path_Integral");

//#pragma omp parallel for num_threads(5) default(none) shared(ctrl_samples, samples, normX_cholesk)
    ...
    int t; int sample;
#pragma omp parallel for num_threads(10) default(none) private(t, sample) shared(step, cost, cst, data_vec, ctrl_samples, samples)
    for(sample = 0; sample < samples; ++sample)
    {
        data_vec[sample].u = CtrlVec::Zero(); data_vec[sample].x = StateVec::Zero();
        for(t = 0; t < time; ++t)
        {
            data_vec[sample].u =  ctrl_samples.block(sample, t, 1, 1);
            step(data_vec[sample]);
            cst[sample][0] = cost(data_vec[sample]);
        }
    }
